...
public class MyController {
    ...
    public String deleteGroupTest(@PathVariable(value = "id") long groupId,
                                  Model model) {
        contactService.deleteGroup(groupId);
        return "redirect:/";
    }

    @GetMapping(value = "/allPictures")
    public String infoAllPictures(Model model,
                                  @RequestParam(required = false, defaultValue = "0") Integer page) {
        if (page < 0) page = 0;

        List<Picture> pictures = contactService
                .findAllPictures(PageRequest.of(page, ITEMS_PER_PAGE, Sort.Direction.DESC, "id"));
    ...
    public ResponseEntity<Void> deletePictures(@RequestParam(value = "toDeletePictures[]", required = false) long[] toDeletePictures) {
        if (toDeletePictures != null && toDeletePictures.length > 0)
            contactService.deletePictures(toDeletePictures);
